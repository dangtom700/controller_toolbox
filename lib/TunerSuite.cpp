...
enum class Tier { IDEAL, SOFT, FALLBACK };
...
static Tier tier(const char* tuner, CtrlKind k)
{
    ...
    if (!strcmp(tuner, "RelayZN")) {
        switch (k) {
        case CtrlKind::PID:     return Tier::IDEAL;
        case CtrlKind::Smith:   return Tier::SOFT;    // tune the inner PID
        case CtrlKind::LeadLag: return Tier::SOFT;    // Ku/Tu can give a gain hint
        default:                return Tier::FALLBACK;
        }
    }
    ...
    if (!strcmp(tuner, "IMCPID")) {
        switch (k) {
        case CtrlKind::PID:     return Tier::IDEAL;
        case CtrlKind::Smith:   return Tier::SOFT;
        case CtrlKind::LeadLag: return Tier::SOFT;    // Kp → gain K hint
        default:                return Tier::FALLBACK;
        }
    }
    ...
    if (!strcmp(tuner, "Bryson")) {
        switch (k) {
        case CtrlKind::LQR:  return Tier::IDEAL;
        case CtrlKind::LQG:  return Tier::IDEAL;
        case CtrlKind::MPC:  return Tier::SOFT;   // rho_y/rho_u analogy
        case CtrlKind::SMC:  return Tier::SOFT;   // Q ratio → c_e/c_de hint
        default:             return Tier::FALLBACK;
        }
    }
    ...
    if (!strcmp(tuner, "KalmanNoise")) {
        switch (k) {
        case CtrlKind::LQG:     return Tier::IDEAL;
        case CtrlKind::Generic: return Tier::SOFT;   // standalone KalmanFilter
        default:                return Tier::FALLBACK;
        }
    }
    ...
    if (!strcmp(tuner, "MPCHorizon")) {
        switch (k) {
        case CtrlKind::MPC: return Tier::IDEAL;
        case CtrlKind::LQR: return Tier::SOFT;   // settling-time info is still useful
        default:            return Tier::FALLBACK;
        }
    }
    ...
    if (!strcmp(tuner, "LoopShaping")) {
        switch (k) {
        case CtrlKind::LeadLag: return Tier::IDEAL;
        case CtrlKind::PID:     return Tier::SOFT;   // omega_c → bandwidth / N hint
        default:                return Tier::FALLBACK;
        }
    }
...
{
    switch (k) {
    case CtrlKind::PID:     return "DiscretePID";
    case CtrlKind::LQR:     return "DiscreteLQR";
    case CtrlKind::LQG:     return "DiscreteLQG";
    case CtrlKind::MPC:     return "DiscreteMPC";
    case CtrlKind::LeadLag: return "DiscreteLeadLag";
    case CtrlKind::SMC:     return "DiscreteSMC";
    case CtrlKind::ADRC:    return "DiscreteADRC";
    case CtrlKind::ESC:     return "ExtremumSeeker";
    case CtrlKind::Smith:   return "SmithPredictor";
    case CtrlKind::Generic: return "IController (generic)";
    }
    return "Unknown";
}
...
{
    switch (k) {
    case CtrlKind::PID:     return "StepResponseTuner (IMC) or ZieglerNicholsTuner";
    case CtrlKind::LQR:     return "LQRWeightTuner::brysonMethod(xmax, umax)";
    case CtrlKind::LQG:     return "LQRWeightTuner::brysonMethod + KalmanWeightTuner::isotropic";
    case CtrlKind::MPC:     return "MPCHorizonTuner::recommend(plant, Ts)";
    case CtrlKind::LeadLag: return "LoopShapingTuner::tuneImpl(omega_c, phase_add, gain)";
    case CtrlKind::SMC:     return "set c_e, K, phi directly from sliding-surface theory";
    case CtrlKind::ADRC:    return "set omega_c, omega_o, b0 via bandwidth parameterisation";
    case CtrlKind::ESC:     return "set perturbFreq >> plant bandwidth; use TunerSuite::optimise";
    case CtrlKind::Smith:   return "tune the inner DiscretePID, not SmithPredictor itself";
    case CtrlKind::Generic: return "TunerSuite::optimise() with a simulation-based cost function";
    }
    return "";
}
    ...
    int*                                              evalCount_out)
{
    ...
    while (evals < maxEvals) {
        ...
        auto [xr, fr] = eval([&]{ // reflect
            ...
            for (int j=0;j<n;++j) v[j] = xo[j] + alpha*(xo[j]-simplex[n].first[j]);
            return v; }());
        ++evals;
        ...
        if (fr < simplex[0].second) {
            ...
            if (fc < (outer ? fr : simplex[n].second)) {
                ...
                for (int i = 1; i <= n && evals < maxEvals; ++i) {
                    std::vector<double> v(n);
                    for (int j=0;j<n;++j) v[j]=best[j]+sigma*(simplex[i].first[j]-best[j]);
                    simplex[i] = eval(v);
                    ++evals;
                }
            }
    ...
    if (evalCount_out) *evalCount_out = evals;
    ...
    std::function<std::unique_ptr<IController>(const std::vector<double>&)> factory)
{
    return [plant, ref, N, factory](const std::vector<double>& params) -> double {
        ...
        for (int k = 0; k < N; ++k) {
            double e = ref - y;
            double u;
            try { u = ctrl->compute(e); } catch (...) { return 1e30; }
            if (!std::isfinite(u)) return 1e30;

            Eigen::VectorXd uv(1); uv << u;
            Eigen::VectorXd yv = ssStep(p2, x, uv);
            y = yv(0);
            if (!std::isfinite(y)) return 1e30;
