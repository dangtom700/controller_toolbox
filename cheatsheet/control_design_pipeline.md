...
- Uncertainty representation (extracting multiplicative/additive weight for H∞).
...
Based on the requirements, plant complexity, and implementation constraints, you must **pick the controller architecture** (PID, LQG, MPC, H∞, fuzzy, adaptive…). This is not a tuning step but a critical architectural decision:
...
- **Structured singular value (μ)** if you have a rigorous uncertainty model.
...
[Controller Design & Tuning] (using model + specs)
    ↓
[Robustness & Worst‑Case Analysis] (Monte Carlo, μ) ↺ (if fail, back to specs or controller)
    ↓
