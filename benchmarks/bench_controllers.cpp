...
ctrl::StateSpace makePlant(int n, double Ts = 0.01)
{
    ...
    for (int i = 0; i < n; ++i)     A(i, i)     -= 1.0; // stable diagonal

    Eigen::MatrixXd B(n, 1); B.setZero(); B(n - 1, 0) = 1.0;
    Eigen::MatrixXd C(1, n); C.setZero(); C(0, 0)     = 1.0;
    Eigen::MatrixXd D(1, 1); D.setZero();
...
int main()
{
    ...
    for (int n : {2, 4, 8}) {
        ...
        if (n == 2) {
            ctrl::PIDParams pp; pp.Kp = 1.0; pp.Ki = 0.1; pp.Kd = 0.01; pp.N = 10.0;
            ctrl::DiscretePID pid(pp, Ts);
            printResult(bench("DiscretePID", STEPS, 0.1,
                [&](int) { return pid.compute(1.0); }));

            ctrl::LeadLagParams lp; lp.gain = 2.0; lp.continuousZero = 1.0;
                                    lp.continuousPole = 10.0;
            ctrl::DiscreteLeadLag ll(lp, Ts);
            printResult(bench("DiscreteLeadLag", STEPS, 0.1,
                [&](int) { return ll.compute(1.0); }));

            ctrl::SMCParams sp; sp.K = 5.0; sp.phi = 0.5;
            ctrl::DiscreteSMC smc(sp, Ts);
            printResult(bench("DiscreteSMC", STEPS, 0.1,
                [&](int) { return smc.compute(1.0); }));

            ctrl::ADRCParams ap; ap.omega_o = 10.0; ap.omega_c = 3.0; ap.b0 = 1.0;
            ctrl::DiscreteADRC adrc(ap, Ts);
            printResult(bench("DiscreteADRC", STEPS, 0.1,
                [&](int k) { return adrc.computeTracking(0.0, 1.0); }));

            ctrl::ESCParams ep; ep.dither_amp = 0.05; ep.dither_freq = 5.0;
                                ep.lpf_cutoff = 1.0; ep.hpf_cutoff = 0.5;
                                ep.integrator_gain = 0.1;
